...
class InFileStream final : public IInStream, public IStreamGetSize
{
    long m_Ref{1};
  ...
  public:
    ...
    static HRESULT OpenPath(LPCWSTR pszFile, InFileStream **pobj) noexcept
    {
        ...
        HRESULT hr = SHCreateStreamOnFileEx(pszFile, STGM_READ, FILE_ATTRIBUTE_NORMAL, FALSE, nullptr, &stream);
        if (FAILED(hr))
            return hr;
    ...
    {
        return m_baseStram->Seek(LARGE_INTEGER{.QuadPart = offset}, seekOrigin,
                                 reinterpret_cast<ULARGE_INTEGER *>(newPosition));
    }
...
class InSafeArrayStream final : public IInStream, public IStreamGetSize
{
    long m_Ref{1};
    ULONG m_offset{0};
...
class SafeArrayStream final : public ISafeArrayStream
{
    long m_Ref{1};
    ULONG m_offset{0};
  ...
  public:
    static HRESULT From(_In_opt_ LPSAFEARRAY psa, __RPC__deref_out_opt ISafeArrayStream **ppstrm)
    {
